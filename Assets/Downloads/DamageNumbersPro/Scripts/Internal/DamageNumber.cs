...
namespace DamageNumbersPro
{
    public abstract class DamageNumber : MonoBehaviour
    {
        ...
        /// </summary>
        /// <returns>The spawned damage number. A clone of the original which can be modified at runtime.</returns>
        public DamageNumber Spawn(Vector3 newPosition, BigInteger newNumber)
        {
            DamageNumber newDN = Spawn();
            //Position:
            newDN.SetPosition(newPosition);
        ...
        /// </summary>
        /// <returns>The spawned damage number. A clone of the original which can be modified at runtime.</returns>
        public DamageNumber Spawn(Vector3 newPosition, long newNumber, Transform followedTransform)
        {
            DamageNumber newDN = Spawn();
        ...
        /// </summary>
        /// <returns>The spawned damage number. A clone of the original which can be modified at runtime.</returns>
        public DamageNumber Spawn(Vector3 newPosition, long newNumber, Color newColor)
        {
            DamageNumber newDN = Spawn();
        ...
        /// </summary>
        /// <returns>The spawned damage number. A clone of the original which can be modified at runtime.</returns>
        public DamageNumber Spawn(Vector3 newPosition, long newNumber, Transform followedTransform, Color newColor)
        {
            DamageNumber newDN = Spawn();
        ...
        public void SetGradient(TMP_ColorGradient newGradient)
        {
            //References:
            GetReferencesIfNecessary();
            ...
            foreach (TMP_Text tmp in GetTextMeshs())
            {
                tmp.colorGradientPreset = newGradient;
            }
        }
        ...
        public void UpdateText()
        {
            ...
            if (enableNumber)
            {
                ...
                numberString = AddUnit(number, 7);
                // if (digitSettings.decimals <= 0)
                // {
                //     //numberString = ProcessIntegers(Math.Round(number).ToString(), out shortened);
                // }
                // else
                // {
                //     string allDigits = Math.Round(number * Mathf.Pow(10, digitSettings.decimals)).ToString();
                ...
                {
                    numberScale = scaleByNumberSettings.fromScale + (scaleByNumberSettings.toScale - scaleByNumberSettings.fromScale) * Mathf.Clamp01(((float)number - scaleByNumberSettings.fromNumber) / (scaleByNumberSettings.toNumber - scaleByNumberSettings.fromNumber));
                }
            }
            ...
            float minScale = renderThroughWallsScale * 0.7f;
        ...
        public static string AddUnit(BigInteger number, int digit)
        {
            ...
            return (long)((float)number / Math.Pow(10, unit * 3)) + units[unit];
        }
        ...
        void GiveNumber(float time)
        {
            if (!givenNumber)
            {
                ...
                myAbsorber.OnAbsorb((float)number, (float)myAbsorber.number);
            }
        }
