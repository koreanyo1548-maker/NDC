...
namespace dynamicscroll
{
    ...
    {      
        ...
        protected VerticalLayoutGroup mVerticalLayoutGroup;
        protected HorizontalLayoutGroup mHorizontalLayoutGroup;
        protected GridLayoutGroup mGridLayoutGroup;
        protected ContentSizeFitter mContentSizeFitter;
		...
		protected IList<T> infoList;
        private Tween forceMoveTween;
        public IList<T> RawDataList => infoList;
		public void Initiate(DynamicScrollRect scrollRect, IList<T> infoList, int startIndex, string objReference, bool createMoreIfNeeded = true, int? forceAmount = null)
        {
		...
		public virtual void ChangeList(IList<T> infoList, int startIndex = -1, bool resetContentPosition = false)
        {
	        if (startIndex == -1)
	        {
        ...
        protected void CreateList(int startIndex)
		{
			var totalSize = 0f;
        ...
        public void ForceUpdate(int curIdx)
        {
	        var currentObject = objectPool.Find(x => x.CurrentIndex == curIdx);
		    currentObject.UpdateScrollObject(infoList[curIdx], curIdx);
        }

        public void ForceUpdateList()
        {
	        var lowest = GetLowest().CurrentIndex;
	        var highest = GetHighest().CurrentIndex;
	        if (lowest < highest) (lowest, highest) = (highest, lowest); 
	        while (highest <= lowest) ForceUpdate(highest++);
        }
        ...
        protected virtual void DisableGridComponents()
        {
        ...
        public void MoveToIndex(int index, float? totalTime = null, float? timePerElement = null)
        {
	        ...
	        if (index >= infoList.Count)
		        index = infoList.Count - 1;//throw new Exception("Invalid index to move: " + index);
            ...
            forceMoveTween = (mIsHorizontal ? ScrollRect.content.DOAnchorPosX(pos, time) : ScrollRect.content.DOAnchorPosY(pos, time)).SetEase(Ease.OutQuint).OnComplete(
	            () =>
	            {
					UpdateObjectsCentralizedPosition();
	            });
