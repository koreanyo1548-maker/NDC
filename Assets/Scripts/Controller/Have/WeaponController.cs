using System.Collections.Generic;
using Controller.Infos;
using Controller.Play;
using Data;
using Data.DbEquipment;
using Data.DbStage;
using Data.DbSummon;
using Data.DbUser.Equipment;
using Managers;
using ThirdParty;
using Utils;
using Random = UnityEngine.Random;

namespace Controller.Have
{
    public class WeaponController: Singleton<WeaponController>
    {
        public bool AnyThingToMerge()
        {
            return DbUserWeapon.HaveAny(weapon => weapon.CanMerge());
        }

        public void MergeAll()
        {
            var log = string.Empty;
            DbUserWeapon.ForEach(MergeToMax);
            PlayFabManager.Store.SetLog(log);

            void MergeToMax(DbUserWeapon w)
            {
                // if (!SettingController.data.WeaponMergeAll.Value && w.Awakening.Value < 7) return;
                if (DbWeapon.Get(w.Id).FullGrade == FullGradeType.Mythic5) return;
                
                var mergeCount = w.Count.Value / 5;
                if (mergeCount < 1) return;
                
                var merged = DbUserWeapon.Get(w.Id+1);
                var prev = merged.Count.Value;
                merged.Count.Value += mergeCount;
                w.Count.Value -= 5 * mergeCount;

                QuestController.I.DoQuests(QuestType.WeaponMerge, mergeCount);
                // PlayFabManager.Store.Save(PlayFabStore.SaveType.Weapon);
                log = $"[무기합성] {w.Id}를 {mergeCount}개 합성 {merged.Id}가 {prev}개 => {merged.Count.Value}개    |    ";
            }
        }

        

        public List<IDbCanSummon> AddRandom(int count)
        {
            var totalCount = count;
            var added = new Dictionary<FullGradeType, int>();
            var result = new List<IDbCanSummon>();
            var prMeta = DbSummonGradeProbability.Get(LevelController.I.GetSummonLevelForMeta(SummonType.Weapon));
            while (count-- > 0)
            {
                var pick = FullGradeType.Normal1;
                var gradeChoose = Random.Range(0, 1000000);
                var pr = 0;
                while (gradeChoose >= pr)
                {
                    pr += prMeta.GetPr(pick);
                    pick+=5;
                }
                var add = pick - 5;

                var pickNumber = 0;
                pr = 0;
                var numberPr = DbSummonNumberProbability.Get(GetGradeType(add)).Probability;
                var numberChoose = Random.Range(0, 1000000);
                while (numberChoose >= pr)
                {
                    pr += numberPr[pickNumber];
                    pickNumber++;
                }

                add += pickNumber - 1;

                if (added.ContainsKey(add)) added[add]++;
                else added.Add(add, 1);
                result.Add(DbWeapon.Get(w => w.FullGrade == add));
            }
            foreach (var add in added)
            {
                var w = DbUserWeapon.Get(w => w.Meta.FullGrade == add.Key);
                w.Count.Value += add.Value;
            }
            QuestController.I.DoQuests(QuestType.WeaponSummon, totalCount);

            return result;
            
            GradeType GetGradeType(FullGradeType grade)
            {
                if (grade >= FullGradeType.Mythic1) return GradeType.Mythic;
                if (grade >= FullGradeType.Legendary1) return GradeType.Legendary;
                if (grade >= FullGradeType.Heroic1) return GradeType.Heroic;
                if (grade >= FullGradeType.Unique1) return GradeType.Unique;
                if (grade >= FullGradeType.Rare1) return GradeType.Rare;
                if (grade >= FullGradeType.Magic1) return GradeType.Magic;
                return GradeType.Normal;
            }
        }

        public void Add(int weaponId, int count)
        {
            DbUserWeapon.Get(weaponId).Count.Value += count;
        }

        public long GetMonsterKillReward()
        {
            var stageReward = DbStageReward.Get(LevelController.data.Stage.Value);

            var haveReward = stageReward.Weapons.Count > 0 && Random.Range(0, 100) < (stageReward.WeaponProbability * TotalStatController.I.GetStat(StatType.StageItemRate) * 0.01f);
            if (haveReward)
            {
                var reward = stageReward.Weapons[Random.Range(0, stageReward.Weapons.Count)];
                
                var w = DbUserWeapon.Get(reward);
                w.Count.Value += 1;
                Manager.UI.RewardLog.Add(CurrencyType.Weapon, 1, w.Id);
                return 1;
            }
            return 0;
        }
        
        public int GetTotalAwakeningCount()
        {
            var count = 0;
            DbUserWeapon.ForEach(w => count += w.Awakening.Value);
            return count;
        }

        
        public int GetTotalGrowthCount()
        {
            var count = 0;
            DbUserWeapon.ForEach(w => count += (w.Growth.Value > 0 ? w.Growth.Value-1 : 0));
            return count;
        }

        // public int GetPower()
        // {
        //     var sum = 0;
        //     DbUserWeapon.ForEach(w => sum += w.Power.Value);
        //     return sum;
        // }
    }
}